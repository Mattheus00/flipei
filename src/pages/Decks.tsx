    ...
    const fetchDecks = async () => {
        try {
            ...
            const formattedDecks = data.map((d: any) => ({
                ...
                last_review: 'Nunca', // Placeholder logic for now
                progress: 0 // Placeholder logic for now
            }))
        ...
        <div className="space-y-8 animate-fade-in font-dm text-white relative">
            ...
            <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full bg-[#0D1829] border-2 border-white/5 rounded-2xl pl-12 pr-6 py-4 outline-none focus:border-[#1A6BFF]/40 text-white placeholder:text-gray-700 transition-all font-bold"
                    />
                </div>
                <div className="flex gap-2">
                    <button className="bg-[#0D1829] border-2 border-white/5 p-4 rounded-2xl text-gray-500 hover:text-white transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {decks.map((deck) => (
                    <div
                        key={deck.id}
                        className="group bg-[#0D1829] border-2 border-white/5 rounded-[2.5rem] overflow-hidden hover:border-[#1A6BFF]/40 transition-all hover:-translate-y-2 relative"
                    >
                        {/* Top Bar with Deck Color */}
                        <div className="h-3 w-full opacity-80" style={{ backgroundColor: deck.color }}></div>
                        ...
                        <div className="p-8">
                            ...
                            <div className="mt-8 flex gap-3">
                                <Link
                                    to={`/study/${deck.id}`}
                                    className="flex-1 bg-[#1A6BFF] text-white hover:bg-indigo-500 py-3.5 rounded-2xl font-black text-sm text-center transition-all shadow-lg active:scale-95"
                                >
                                    Estudar Agora
                                </Link>
                        ...
                        <form onSubmit={handleCreateDeck} className="space-y-8">
                            <div className="space-y-3">
                                ...
                                <input
                                    type="text"
                                    required
                                    placeholder="Ex: Anatomia, Direito Civil..."
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-3">
                                    ...
                                    <input
                                        type="text"
                                        className="w-full bg-[#111F35] border-2 border-white/5 rounded-2xl px-6 py-4 outline-none text-2xl text-center"
                                        value={newDeck.emoji}
                                        onChange={e => setNewDeck({ ...newDeck, emoji: e.target.value })}
                                    />
                                </div>
                                <div className="space-y-3">
                                    ...
                                    <input
                                        type="color"
                                        className="w-full h-[64px] bg-[#111F35] border-2 border-white/5 rounded-2xl p-2 cursor-pointer"
                                        value={newDeck.color}
                            ...
                            <button
                                type="submit"
                                disabled={creating}
                                className="w-full bg-[#1A6BFF] hover:bg-indigo-500 text-white py-5 rounded-2xl font-black text-lg transition-all shadow-xl flex items-center justify-center gap-3 disabled:opacity-50"
                            >
